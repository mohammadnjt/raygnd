...
```
- **منطق عملکرد:**
  1. اگر کاربر در سیستم وجود نداشته باشد، به عنوان کاربر جدید با نقش `customer` ایجاد می‌شود. اگر در سیستم باشد، کاربر موجود استفاده می‌شود.
  2. یک شرکت/طلافروشی جدید برای کاربر ثبت می‌شود اما وضعیت آن غیرفعال (`isActive: false`) خواهد بود.
  3. شناسه شرکت (`companyId`) در پاسخ ارسال می‌شود.
  4. اگر کاربر در طول این فرایند (پیش از تایید ادمین) وارد سیستم شود، با نقش `customer` وارد می‌شود.
- **خروجی نمونه:**
```json
{
  "success": true,
  "message": "درخواست معرفی با موفقیت ثبت شد و در انتظار تایید است.",
  "companyId": "650000000000000000000020",
  "data": {
...
```
- **منطق عملکرد:**
  1. شرکت غیرفعال مربوطه فعال می‌شود (`isActive = true`).
  2. نقش کاربر به `gold` یا `lab` تغییر کرده و شرکت به حساب وی لینک می‌شود.
...
```
- **منطق عملکرد:**
  1. شرکت ثبت‌شده غیرفعال و حذف می‌شود.
  2. کاربر هنوز به عنوان `customer` در سیستم باقی می‌ماند.
...
### ثبت درخواست نوبت و سفارش (Request Order / Create Order)
- **آدرس:** `POST /api/general/orders/request` (آدرس‌های جایگزین: `POST /api/general/orders/create` یا `POST /api/general/orders`)
- **نیاز به توکن:** دارد
- **ورودی (JSON):**
```json
{
  ...
  "weight": 50.5,
  "hasJewel": false,
  "sellerName": "طلافروشی احمدی",
  "sellerPhone": "09121111111",
  "manual": true
}
```
- **منطق عملکرد:**
  1. **افزودن دستی توسط آزمایشگاه:** هنگامی که آزمایشگاه به صورت دستی اقدام به ثبت سفارش می‌کند، `companyId` (یا `labId`) آزمایشگاه مربوطه ارسال می‌شود و وضعیت سفارش به صورت `manual: true` تنظیم می‌گردد.
  2. **ثبت نوبت توسط طلافروش:** هنگامی که طلافروش اقدام به ثبت نوبت می‌کند، `companyId` یا `labId` آزمایشگاه هدف مشخص گردیده و سیستم اطلاعات طلافروش را از حساب کاربری متوجه شده و وضعیت سفارش به صورت `manual: false` خواهد بود.
- **نمونه خروجی:**
```json
{
  "success": true,
...
### دریافت ۱۰ کد انگ پرجستجو (Top Searched Ang Codes - مخصوص سوپر ادمین)
- **آدرس:** `GET /api/admin/top-searched-angs` یا `GET /api/admin/top-searched`
- **نیاز به توکن:** دارد (نقش کاربر باید `superAdmin` باشد)
- **توضیحات:** لیست ۱۰ کد انگی که بیشترین تعداد استعلام و سرچ توسط کاربران را در سیستم داشته‌اند همراه با تعداد تکرار و اطلاعات پایه (نام آزمایشگاه، عیار، مشتری) برمی‌گرداند.
- **نمونه خروجی:**
```json
{
  "success": true,
...
### دریافت لیست طلافروشان (Goldsmiths / Sellers - مخصوص آزمایشگاه‌ها)
- **آدرس:** `GET /api/general/goldsmiths` (یا `GET /api/general/sellers` یا `GET /api/general/labs/goldsmiths`)
- **نیاز به توکن:** دارد
- **پارامترها (Query Parameters):**
  - `search` یا `q` یا `name` یا `phone` یا `mobile` (اختیاری): متن یا شماره تلفن برای جستجو بر اساس نام، نام خانوادگی، شماره موبایل یا تلفن طلافروش.
  - `page` (اختیاری، پیش‌فرض 1): شماره صفحه.
  - `limit` (اختیاری، پیش‌فرض 20): تعداد آیتم‌ها در هر صفحه.
- **توضیحات:** لیست طلافروشان/فروشندگان ثبت‌شده در سیستم و فروشندگانی که به آزمایشگاه سفارش داده‌اند را برمی‌گرداند. امکان جستجو بر اساس نام یا شماره تماس وجود دارد.
- **نمونه خروجی:**
```json
{
  "success": true,
  "data": [
    {
      ...
      "companyId": "650000000000000000000010",
      "name": "احمد احمدی",
      "fname": "احمد",
      "lname": "احمدی",
      "mobile": "09121111111",
